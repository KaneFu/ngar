...
def prob(alpha_matrix,bin_num,shift_num):
    ...
    rank_matrix = np.array([rank_ser[i].labels for i in xrange(len(rank_series))])
    ...
    for ii in range(prob_matrix):
        for jj in range(prob_matrix[0]):
